...
function do_exit() {
    ...
    if [ "${status}" -ne 0 ]; then
        ...
        if [ $role = "Compute" ] || [ $role = "All-In-One" ]; then
            ...
            rm /etc/sysconfig/network-scripts/route-dmz
        fi
        ...
        yum -y remove `rpm -qa | grep openstack-nova`
...
function dmz_routes {
    ...
    local dmzfile=/etc/sysconfig/network-scripts/ifcfg-${net_bridge}
...
EOF
    ...
    if ! ls /etc/sysconfig/network-scripts/ifcfg-chefbr0; then
        cat >> /etc/sysconfig/network-scripts/ifcfg-chefbr0 << EOF
DEVICE="chefbr0"
NM_CONTROLLED="no"
ONBOOT="yes"
...
function download_cookbooks {
    ...
    if [ ! -e /opt/rpcs/cookbooks ]; then
        run_twice git clone http://github.com/williamherry/openstack-cookbooks /opt/rpcs/chef-cookbooks 1>/dev/null
...
function setup_chef_initscripts() {
    ...
    cp `find /opt -name chef-client | grep redhat | grep init` /etc/init.d/
    cp `find /opt -name chef-client | grep redhat | grep sysconfig` /etc/sysconfig/
...
function apt_it_up() {
    do_substatus 33 "running yum update" "apt"
    run_twice yum -y update 1>/dev/null
...
function prepare() {
    ...
    cp /opt/rpcs/openstack-deploy/ssh-copy-id /usr/bin/
}
